document.addEventListener('DOMContentLoaded', () => {
    ...
    addCentreForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('http://localhost:3000/admin/addCentre', {
          ...
          body: JSON.stringify({ name, address, workingHours}),
        });
  ...
  if (removeCentreForm) {
    removeCentreForm.addEventListener('submit', async (e) => {
      ...
      const nameInput = document.getElementById('remove-centre-name');
      const addressInput = document.getElementById('remove-centre-address');
  ...
  function displayDosageDetails(dosageDetails) {
    ...
    if (dosageDetails.length > 0) {
      dosageDetails.forEach((centre) => {
        const { name, address, workingHours, slotsAvailable } = centre;
        ...
        centreElement.innerHTML = `
          ...
          <p>Available Slots: ${slotsAvailable}</p>
          <hr>
        `;
  ...
  function displayCentres(centres) {
    ...
    if (centres.length > 0) {
      centres.forEach((centre) => {
        const { _id, name, address, workingHours, slotsAvailable } = centre;
