...
app.post('/slots', async (req, res) => {
  try {
    ...
    if (centre.availableVaccines.length === 0) {
      return res.status(400).json({ error: 'No vaccines available' });
    }
    ...
    const selectedVaccine = centre.availableVaccines.pop();
    centre.slotsAvailable--;
    centre.bookedDate = new Date();
    await centre.save();

    console.log('Vaccination slot booked successfully');
    res.json({ message: 'Vaccination slot booked successfully', vaccine: selectedVaccine });
  } catch (error) {
...
app.post('/admin/addCentre', isAdmin, async (req, res) => {
  const { name, address, workingHours, availableVaccines } = req.body;
  try {
    const newCentre = new Centre({ name, address, workingHours, availableVaccines,slotsAvailable: 10 });
...
app.post('/admin/removeCentre', isAdmin, async (req, res) => {
  ...
  try {
    const centre = await Centre.findOneAndDelete({ name, address });
    ...
    const dosageDetails = await Centre.aggregate([
      {
        $group: {
          ...
          remainingDoses: { $sum: { $arrayElemAt: ['$availableVaccines', 0] } },
        },
      },
      {
        $project: {
